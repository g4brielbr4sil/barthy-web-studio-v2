...
const OFFICIAL_WHATSAPP_MESSAGE =
  'Olá! Conheci o trabalho de vocês pelo site da *Barthy Web Studio* e tenho um projeto em mente. Poderiam me orientar sobre os próximos passos?'

assert.equal(DEFAULT_WHATSAPP_URL, OFFICIAL_WHATSAPP_URL)
assert.equal(
  new URL(DEFAULT_WHATSAPP_URL).searchParams.get('text'),
